...
if __name__ == "__main__":
    ...
    valid_rows = []
    for line in lines:
        # Only accept rows with exactly N columns (no text, no empty lines)
        cells = [cell.strip() for cell in line.split(',')]
        if len(cells) == len(colnames):  # N = number of columns in your survey
            valid_rows.append(cells)
    df = pd.DataFrame(valid_rows, columns=colnames)
    df.to_csv("synthetic_from_real_schema.csv", index=False)
